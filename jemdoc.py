...
def testeqsupport():
  ...
  if rc != 0:
    ...
    msg += '  latex: ' + p.stdout.readlines()[0].decode('utf-8').rstrip() + '.\n'
  ...
  if rc != 0:
    ...
    msg += '  dvipng: ' + p.stdout.readlines()[0].decode('utf-8').rstrip() + '.\n'
...
def geneq(f, eq, dpi, wl, outname):
  ...
  try:
    ...
    depth = int(p.stdout.readlines()[-1].decode('utf-8').split('=')[-1])
  finally:
    # Clean up.
    for ext in exts:
      g = basefile + ext
